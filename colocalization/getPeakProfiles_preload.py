...
#limit the number of sites to save memory
if len(sites) > K:
    ix = np.sort(random.sample(range(0, len(sites)), K))
    sites = sites[ix]
...
#reload(pp)
peakProfs = pp.getPeakProfilesFromCoverage(sites, coverage, halfwinwidth=3000, stranded=stranded)
