...
for x in `ls -1 split/${FORWARD}.split.*`; do
    ...
    qsub -cwd -l h_vmem=30G -V -N bismark-${MYID} -j y -b y bismark --temp_dir tmp -o bismark --bowtie2 -p $((${NTHREADS}*${DIRECTIONS})) $BISMARK_OPTS $BOWTIE2_OPTS $GENOME_FOLDER $READSTR
done
...
qsub -S /bin/bash -hold_jid bismark-${MYID} -cwd -V -l h_vmem=4G -j y -N merge -b y java -jar $(which MergeSamFiles.jar) $SAMS O=$BAM AS=false SO=coordinate TMP_DIR=./tmp CREATE_INDEX=true
